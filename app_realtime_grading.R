...
#### Install package worms separately from CRAN archive
...
#GET FILES FROM GITHUB (not needed for this version)
...
grades_checklist<-function(checklist,inputz,coordz){
  ...
  taxon2.5<-rbind(taxon2,remaining_bins2)
  ...
  taxon2.5<-left_join(taxon2.5,bps,by="species_name")
  #taxon2$bin_uri.y=NULL
  #names(taxon2)[names(taxon2) == "bin_uri.x"] <- "bin_uri"
  taxon2.5<-left_join(taxon2.5,spb,by="bin_uri")
  ...
  taxon2.9<-taxon2.5[taxon2.5$processid %in% taxon2$processid,]
  taxon3<-taxon2.9[taxon2.9$markercode=="COI-5P",]
