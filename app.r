...
grades2<-function(groups){
  ...
  taxon2<-taxon2[taxon2$species_name!=""|is.na(taxon2$species_name),]
  ...
  species_list<-as.list(as.character(unique(taxon3$species_name)))
  e<-lapply(species_list, function (x) bold_stats(taxon=x))
  f<-lapply(e, '[[', c('bins','count'))
  g<-unlist(f)
  g=data.frame(g)
  h<-cbind(unique(taxon3$species_name),g$g)
  h<-data.frame(h)
  names(h)<-c("species_name","bold_bin_per_species")
  taxon3<-inner_join(taxon3,h)
  taxon6=taxon3
  taxon7=taxon6
  taxon8<-data.frame(taxon7$species_name,taxon7$bin_uri,taxon7$nucleotides,taxon7$country,taxon7$family_name,taxon7$order_name,taxon7$class_name,taxon7$sampleid,taxon7$processid,taxon7$bold_species_per_bin,taxon7$bold_bin_per_species, taxon7$lat,taxon7$lon)
  names(taxon8)<-c("species_name","BIN","nucleotides","country","family","order","class","sampleid","processid","bold_species_per_bin","bold_bin_per_species","lattitude","longitude")
  num_species=table(taxon8$species_name)
  num_species=as.data.frame(num_species)
  names(num_species)=c("species","frequency_species")
  taxon8$grade=NA
  names(taxon8)=c("species","BIN","sequence","country","family","order","class","sampleid","processid","bold_species_per_bin","bold_bin_per_species","lattitude","longitude","grade")
  taxon9<-inner_join(taxon8,num_species)
  taxon9$base_number=str_count(taxon9$sequence, pattern="[A-Z]")
  taxon9=data.frame(taxon9$species,taxon9$BIN,taxon9$sequence,taxon9$country,taxon9$grade,taxon9$frequency_species,taxon9$base_number,taxon9$family,taxon9$order,taxon9$class,taxon9$sampleid,taxon9$processid,taxon9$bold_species_per_bin,taxon9$bold_bin_per_species,taxon9$lat,taxon9$lon)
  names(taxon9)=c("species","BIN","sequence","country","grade","species_frequency","base_number","family","order","class","sampleid","processid","bold_species_per_bin","bold_bin_per_species","lattitude","longitude")
  taxon9$bold_bin_per_species=as.numeric(levels(taxon9$bold_bin_per_species))[taxon9$bold_bin_per_species]
  taxon9$bold_species_per_bin=as.numeric(levels(taxon9$bold_species_per_bin))[taxon9$bold_species_per_bin]
  taxon10<-taxon9 %>%
    mutate(grade = ifelse(bold_species_per_bin>1,"E",
                          ifelse(species_frequency<3,"D",
                                 ifelse(bold_bin_per_species>1 & bold_species_per_bin==1,"C",
                                        ifelse(bold_bin_per_species==1 & bold_species_per_bin==1 & species_frequency<11,"B",
                                               ifelse(bold_bin_per_species==1 & bold_species_per_bin==1 & species_frequency>10,"A",NA)) ))))
  taxon10<-taxon10 %>% left_join(taxon10 %>% 
                                   group_by(species) %>% 
  ...
  taxon10$sum_b=NULL
  taxon10$sum_e=NULL
  taxon10$BIN_per_species=NULL
  taxon10$species_per_bin=NULL
  taxon10$species_frequency=NULL
  taxon10$bold_species_per_bin=NULL
  taxon10$bold_bin_per_species=NULL
  assign('taxon10',taxon10,envir=.GlobalEnv)
}
...
#Specimens
plot_summary_specimens=function(taxon10){
  frequency_grades=c(length(which(taxon10$grade=="A")),length(which(taxon10$grade=="B")),length(which(taxon10$grade=="C")),length(which(taxon10$grade=="D")),length(which(taxon10$grade=="E")))
...
#Species
plot_summary_species=function(taxon10){
  frequency_grades=c(as.numeric(length(unique(taxon10$species[taxon10$grade=="A"]))),as.numeric(length(unique(taxon10$species[taxon10$grade=="B"]))),as.numeric(length(unique(taxon10$species[taxon10$grade=="C"]))),as.numeric(length(unique(taxon10$species[taxon10$grade=="D"]))),as.numeric(length(unique(taxon10$species[taxon10$grade=="E"]))))
  names_grades=c("A","B","C","D","E")
  dataframe_grades=data.frame(names_grades,frequency_grades)
...
#grade A
create_A=function(taxon10){
  taxon_a=taxon10[taxon10$grade=="A",]
...
#grade B
create_B=function(taxon10){
  taxon_b=taxon10[taxon10$grade=="B",]
...
#grade C
create_C=function(taxon10){
  taxon_c=taxon10[taxon10$grade=="C",]
...
#grade D
create_D=function(taxon10){
  taxon_d=taxon10[taxon10$grade=="D",]
...
#grade E
create_E=function(taxon10){
  taxon_e=taxon10[taxon10$grade=="E",]
...
#Only grades A and B
create_AB=function(taxon10){
  taxon_ab=taxon10[taxon10$grade=="A" | taxon10$grade=="B",]
...
create_ABC=function(taxon10){
  taxon_abc=taxon10[taxon10$grade=="A" | taxon10$grade=="B" | taxon10$grade=="C",]
...
create_ABCD=function(taxon10){
  taxon_abcd=taxon10[taxon10$grade=="A" | taxon10$grade=="B" | taxon10$grade=="C" | taxon10$grade=="D",]
...
#All grades
create_fasta=function(taxon10){
  taxon10$name=paste(taxon10$species, taxon10$BIN,taxon10$processid, sep=" | ")
  taxon20<-data.frame(taxon10$name,taxon10$COI_sequence)
...
####HOME TAB
  ...
  tags$div(style="text-align:justify",tags$h3(tags$strong("Disclaimer")),
           tags$h4(tags$p("Despite the fact that utmost care has been taken by us to guarantee the effectivness and reliability of the web application,
                          the use of the application is without any kind of warranty, expressed or implied. In no event shall the authors be liable for any damages of any type."))))),column(3,align="center",tags$br(),tags$br(),
...
########### DOWNLOAD AND AUDIT DATA SETS TAB
  ...
  tabPanel(title="SELECT TAXA FOR AUDITING",
           ...
           tabsetPanel(type="tabs",tabPanel(tags$span(style="color:#19194d",tags$h4(tags$b("ALL TAXA"))),tags$br(),column(12,align="center",tags$span(icon("globe-africa","fa-3x"),style="color:#000000", tags$h3(align="center",tags$em(tags$strong(tags$u("Download and audit data set for all species"))))),tags$br(),
               textInputAddon(inputId="taxa2",addon=icon("search"),width="500px",label=tags$h5(tags$strong("Enter the name of the taxonomic group or groups separated by commas, without spaces:")),placeholder="Example: Carnivora,Artiodactyla,Soricomorpha"),
...
####### REPORT EVENTS
  summary_reac<-eventReactive(input$clicks3,{
    ...
    tagList(  
      ...
      tags$p(tags$strong("Number of species:"),length(unique(taxon10$species))),
                        tags$p(tags$strong("Number of BINs:"),length(unique(taxon10$BIN))),
      tags$p(tags$strong("Total Number of specimens in reference library:"),length(taxon10$species)),
      tags$br(),
      tags$h4(tags$strong("Specimens")),
      tags$p(tags$strong("Number of specimens with", tags$span(style="color:#1a1aff","A") ,"grade:"),as.numeric(length(taxon10$species[taxon10$grade=="A"]))," 
               ",tags$strong("| Percentage:"),signif((as.numeric(length(taxon10$species[taxon10$grade=="A"])*100)/length(taxon10$species)),digits=3),"%"),
      tags$p(tags$strong("Number of specimens with",  tags$span(style="color:#00b33c","B"), "grade:"),as.numeric(length(taxon10$species[taxon10$grade=="B"]))," ",tags$strong("| Percentage:")
             ,signif((as.numeric(length(taxon10$species[taxon10$grade=="B"])*100)/length(taxon10$species)),digits=3),"%"),
      tags$p(tags$strong("Number of specimens with" ,tags$span(style="color:#cca300","C"), "grade:"),as.numeric(length(taxon10$species[taxon10$grade=="C"]))," ",tags$strong("| Percentage:"),
             signif((as.numeric(length(taxon10$species[taxon10$grade=="C"])*100)/length(taxon10$species)),digits=3),"%"),
      tags$p(tags$strong("Number of specimens with", tags$span(style="color:#ff6600","D"), "grade:"),as.numeric(length(taxon10$species[taxon10$grade=="D"]))," ",tags$strong("| Percentage:"),
             signif((as.numeric(length(taxon10$species[taxon10$grade=="D"])*100)/length(taxon10$species)),digits=3),"%"),
      tags$p(tags$strong("Number of specimens with" ,tags$span(style="color:#cc0000","E") ,"grade:"),as.numeric(length(taxon10$species[taxon10$grade=="E"]))," ",tags$strong("| Percentage:"),
             signif((as.numeric(length(taxon10$species[taxon10$grade=="E"])*100)/length(taxon10$species)),digits=3),"%"),
 
      tags$br(),
      tags$br(),
      tags$h4(tags$strong("Species")),
      tags$p(tags$strong("Number of species with", tags$span(style="color:#1a1aff","A") ,"grade:"),as.numeric(length(unique(taxon10$species[taxon10$grade=="A"])))," 
               ",tags$strong("| Percentage:"),signif((as.numeric(length(unique(taxon10$species[taxon10$grade=="A"]))*100)/length(unique(taxon10$species))),digits=3),"%"),
      tags$p(tags$strong("Number of species with",  tags$span(style="color:#00b33c","B"), "grade:"),as.numeric(length(unique(taxon10$species[taxon10$grade=="B"])))," ",tags$strong("| Percentage:")
             ,signif((as.numeric(length(unique(taxon10$species[taxon10$grade=="B"]))*100)/length(unique(taxon10$species))),digits=3),"%"),
      tags$p(tags$strong("Number of species with" ,tags$span(style="color:#cca300","C"), "grade:"),as.numeric(length(unique(taxon10$species[taxon10$grade=="C"])))," ",tags$strong("| Percentage:"),
             signif((as.numeric(length(unique(taxon10$species[taxon10$grade=="C"]))*100)/length(unique(taxon10$species))),digits=3),"%"),
      tags$p(tags$strong("Number of species with", tags$span(style="color:#ff6600","D"), "grade:"),as.numeric(length(unique(taxon10$species[taxon10$grade=="D"])))," ",tags$strong("| Percentage:"),
             signif((as.numeric(length(unique(taxon10$species[taxon10$grade=="D"]))*100)/length(unique(taxon10$species))),digits=3),"%"),
      tags$p(tags$strong("Number of species with" ,tags$span(style="color:#cc0000","E") ,"grade:"),as.numeric(length(unique(taxon10$species[taxon10$grade=="E"])))," ",tags$strong("| Percentage:"),
             signif((as.numeric(length(unique(taxon10$species[taxon10$grade=="E"]))*100)/length(unique(taxon10$species))),digits=3),"%")
  ...
  barplot1_reac<-eventReactive(input$clicks,{plot_summary_specimens(taxon10)})
  output$bar1<-renderPlot(barplot1_reac(),bg="transparent", execOnResize = TRUE)
  barplot2_reac<-eventReactive(input$clicks2,{plot_summary_species(taxon10)})
  output$bar2<-renderPlot(barplot2_reac(),bg="transparent", execOnResize = TRUE)
  
####### DOWNLOAD REFERENCE LIBRARIES IN FASTA FORMAT
#INDIVIDUAL
  fastaInput <- reactive({create_A(taxon10)})
  fastaInput11 <- reactive({create_B(taxon10)})
  fastaInput12 <- reactive({create_C(taxon10)})
  fastaInput13 <- reactive({create_D(taxon10)})
  fastaInput14 <- reactive({create_E(taxon10)})
...
#GROUPED
  fastaInput2 <- reactive({create_AB(taxon10)})
  fastaInput3 <- reactive({create_ABC(taxon10)})
  fastaInput5 <- reactive({create_ABCD(taxon10)})
  fastaInput4 <- reactive({create_fasta(taxon10)})
