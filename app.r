...
grades2<-function(groups){
  taxon<-bold_seqspec(taxon="Artiodactyla", format = "tsv", marker="COI-5P")
  ...
  taxon7=taxon6
  taxon8<-data.frame(taxon7$species_name,taxon7$bin_uri,taxon7$nucleotides,taxon7$country,taxon7$family_name,taxon7$order_name,taxon7$class_name,taxon7$sampleid,taxon7$processid,taxon7$bold_species_per_bin,taxon7$lat,taxon7$lon)
  names(taxon8)<-c("species_name","BIN","nucleotides","country","family","order","class","sampleid","processid","bold_species_per_bin","lattitude","longitude")
  num_species=table(taxon8$species_name)
  num_species=as.data.frame(num_species)
  names(num_species)=c("species","frequency_species")
  taxon8$grade=NA
  names(taxon8)=c("species","BIN","sequence","country","family","order","class","sampleid","processid","bold_species_per_bin","lattitude","longitude","grade")
  taxon9<-inner_join(taxon8,num_species)
  taxon9$base_number=str_count(taxon9$sequence, pattern="[A-Z]")
  taxon9=data.frame(taxon9$species,taxon9$BIN,taxon9$sequence,taxon9$country,taxon9$grade,taxon9$frequency_species,taxon9$base_number,taxon9$family,taxon9$order,taxon9$class,taxon9$sampleid,taxon9$processid,taxon9$bold_species_per_bin,taxon9$lat,taxon9$lon)
  names(taxon9)=c("species","BIN","sequence","country","grade","species_frequency","base_number","family","order","class","sampleid","processid","bold_species_per_bin","lattitude","longitude")
  taxon10<-taxon9%>% 
    group_by(species) %>%
  ...
  taxon17<-data.frame(taxon16$species,taxon16$BIN.x,taxon16$sequence,taxon16$country,taxon16$grade,taxon16$species_frequency,taxon16$base_number,taxon16$bin_per_species,taxon16$family,taxon16$order,taxon16$class,taxon16$sampleid,taxon16$processid,taxon16$bold_species_per_bin,taxon16$lattitude,taxon16$longitude)
  names(taxon17)<-c("species","BIN","COI_sequence","country","grade","species_frequency","base_number","BIN_per_species","family","order","class","sampleid","processid","bold_species_per_bin","lattitude","longitude")
  taxon18<-full_join(taxon17,taxon11,by="BIN")
  colnames(taxon18)[colnames(taxon18)=="species.x"]<- "species"
  taxon18$species.y<-NULL
  taxon18$bold_species_per_bin=as.numeric(taxon18$bold_species_per_bin)
  taxon19<-taxon18 %>%
...
grades<-function(groups){
  taxon<-bold_seqspec(taxon="hippocampus", format = "tsv", marker="COI-5P")
  taxon2<-taxon[taxon$markercode=="COI-5P",]
  taxon2<-taxon2[taxon2$species_name!=""|is.na(taxon2$species_name),]
  taxon2<-taxon2[!(taxon2$bin_uri == "" | is.na(taxon2$bin_uri)), ]
  taxon2$number<-str_count(taxon2$nucleotides, pattern="[A-Z]")
