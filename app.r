...
grades2<-function(groups){
  ...
  taxon3<-left_join(taxon3,bps,by="species_name")
  taxon3$bin_uri.y=NULL
  names(taxon3)[names(taxon3) == "bin_uri.x"] <- "bin_uri"
  taxon3<-left_join(taxon3,spb,by="bin_uri")
  taxon3$species_name.y=NULL
  names(taxon3)[names(taxon3) == "species_name.x"] <- "species_name"
  ...
  names(taxon8)<-c("species_name","BIN","nucleotides","country","family","order","class","sampleid","processid","species_per_bin","bin_per_species","lattitude","longitude")
